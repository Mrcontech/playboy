...
interface OptimizedHubScreenProps {
  onPlayerSelect?: (player: PlayerComplete) => void;
}

const OptimizedHubScreen = memo(function OptimizedHubScreen({ onPlayerSelect }: OptimizedHubScreenProps) {
  ...
  // TIER 1: Load minimal player data instantly
  const {
    recentPlayers,
  ...
  // Load upcoming dates with optimized caching
  const { 
    data: upcomingDates, 
    loading: datesLoading,
    refetch: refetchDates
  } = useDataLoader({
    key: 'upcoming_dates_hub',
  ...
  // Generate calendar dates with proper date validation
  ...
  const getPlayerName = useCallback((profileId: string | null) => {
    ...
    const player = recentPlayers.find(p => p.id === profileId);
    return player?.name;
  }, [recentPlayers]);
          ...
          <section className="bg-black border-2 border-green-500 rounded-xl p-6">
            ...
            {datesLoading && (
              <div className="text-center mb-4">
                <p className="text-green-400 text-sm animate-pulse font-medium">
                  Loading your upcoming dates...
                </p>
              </div>
            )}
...
export default OptimizedHubScreen;